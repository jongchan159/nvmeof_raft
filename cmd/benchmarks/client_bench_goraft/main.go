...
// Must match field names in goraft.ClientApplyRequest/Response.
...
func printStats(latencies []time.Duration, total time.Duration, n, batch, payload, threads int) {
	...
	fmt.Printf("=== goraft client bench (remote Apply) ===\n")
...
func main() {
	peersStr    := flag.String("peers", "", "comma-separated Raft node addresses")
	...
	nThreads    := flag.Int("threads", 1, "number of concurrent goroutines (each gets its own connection)")
	flag.Parse()

	if *peersStr == "" {
		fmt.Fprintln(os.Stderr, "usage: bench_client_goraft --peers=h1:4020,h2:4021,h3:4022 [--entries=N] [--batch=B] [--payload=P] [--threads=T]")
		os.Exit(1)
	}
